...
if ADM_LEVEL1 != None:
    GADM_TABLE = f"{UC_CATALOG}.{UC_SCHEMA}.wb_boundaries_{COUNTRY_ISO3.lower()}_{ADM_LEVEL1.lower()}_province"
    ...
    GADM_TABLE = f"{UC_CATALOG}.{UC_SCHEMA}.wb_boundaries_{COUNTRY_ISO3.lower()}"
...
selected_wb_boundries_gdf = uc_table_to_gdf(GADM_TABLE)
print(f"  GADM boundaries: {len(selected_wb_boundries_gdf)} features from {GADM_TABLE}")
...
boundary_wkt = selected_wb_boundries_gdf.geometry.unary_union.wkt
aoi_union_geom = selected_wb_boundries_gdf.geometry.unary_union

centroid = selected_wb_boundries_gdf.iloc[0]["geometry"].centroid
...
geo_adm = fl.GeoJson(
    data=selected_wb_boundries_gdf.iloc[0]["geometry"].__geo_interface__,
