...
if ADM_LEVEL1 != None:
    wb_boundry = f"{UC_CATALOG}.{UC_SCHEMA}.wb_boundaries_{ISO_3.lower()}_{ADM_LEVEL1.lower()}_province"
    ...
    lgu_table = f"{UC_CATALOG}.{UC_SCHEMA}.wb_boundaries_lgu_{COUNTRY.lower()}_{ADM_LEVEL1.lower()}_province"
else:
    wb_boundry = f"{UC_CATALOG}.{UC_SCHEMA}.wb_boundaries_{ISO_3.lower()}"
...
print(f"Boundries Table: {wb_boundry}")
...
def extract_health_facilities_osm(iso_2: str, table_name: str, selected_wb_boundry: gpd.GeoDataFrame, adm_level_name: str = "AOI", force: bool = False) -> pd.DataFrame:
    """
    ...
    gdf_health = gdf_health.to_crs(selected_wb_boundry.crs)
    selected_health = gpd.sjoin(gdf_health, selected_wb_boundry, predicate='within')
...
selected_wb_boundry_gdf = extract_boundaries(
    country_iso3= ISO_3,
    adm_level1=ADM_LEVEL1,
    table_name=wb_boundry,
    force=True,
)
...
extract_health_facilities_osm(iso_2= ISO_2, table_name= facilities_table, selected_wb_boundry= selected_wb_boundry_gdf, adm_level_name= ADM_LEVEL1, force=True)
...
print("=" * 60)
print(f"WB boundaries:    {wb_boundry}")
...
print("EXTRACTION COMPLETE")
print("=" * 60)
print(f"Country boundary: {wb_boundry}")
print(f"LGU boundaries:   {lgu_table}  ({len(lgu_gdf)} LGUs)")
print(f"Population raster:     {raster_path}")
print(f"Population table:      {population_table}")
print(f"Health facilities:     {facilities_table}")
print("=" * 60)
